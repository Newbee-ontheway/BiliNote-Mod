...
import { exportFile } from '@/services/downloader'
...
<body>
  ...
  (async () => {
    ...
    mm.fit();
      ...
      // 优先保存到用户指定目录
      try {
        const res = await exportFile({ content: html, filename: `${title || 'mindmap'}.html` });
        if (res?.path) { toast.success(`已导出到: ${res.path}`); return; }
      } catch { }
      // 降级：浏览器下载
  ...
  const exportSvg = async () => {
    try {
      ...
      // 优先保存到用户指定目录
      try {
        const res = await exportFile({ content: svgData, filename: `${title || 'mindmap'}.svg` });
        if (res?.path) { toast.success(`已导出到: ${res.path}`); return; }
      } catch { }
      // 降级：浏览器下载
  ...
  const exportXMind = async () => {
    try {
      ...
      try {
        ...
        const res = await exportFile({ content: b64, filename: `${title || 'mindmap'}.xmind`, is_base64: true });
        if (res?.path) { toast.success(`已导出到: ${res.path}`); return; }
      } catch { }
      // 降级：浏览器下载
      const url = URL.createObjectURL(blob);
    ...
    try {
      ...
      img.onload = async () => {
        try {
          ...
          if (b64) {
            try {
              const res = await exportFile({ content: b64, filename: `${title || 'mindmap'}.png`, is_base64: true });
              if (res?.path) { toast.success(`已导出到: ${res.path}`); return; }
            } catch { }
          }
          // 降级：浏览器下载
