...
class LocalDownloader(Downloader, ABC):
    ...
    def download(
        ...
        if ext_lower in AUDIO_EXTENSIONS:
            # --- 音频文件：直接使用，跳过转换和封面提取 ---
            print(f"[LocalDownloader] 检测到音频文件: {file_name}，跳过转换")
            return AudioDownloadResult(
                ...
                platform="local",
                video_id=title,
                raw_info={'path': video_url, 'source_type': 'audio'},
                video_path=None
            )
        else:
            # --- 视频文件：转换为 mp3 + 提取封面 ---
            ...
            return AudioDownloadResult(
                ...
                platform="local",
                video_id=title,
                raw_info={'path': file_path, 'source_type': 'video'},
                video_path=None
            )
