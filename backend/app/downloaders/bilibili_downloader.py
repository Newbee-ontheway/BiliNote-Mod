...
class BilibiliDownloader(Downloader, ABC):
    ...
    @staticmethod
    def _cookie_string_to_netscape(cookie_string: str, domain: str = ".bilibili.com") -> str:
        """Convert browser cookie string (key=val; key=val) to Netscape format."""
        lines = ["# Netscape HTTP Cookie File", "# Generated by BiliNote"]
    ...
    def _apply_bilibili_cookies(self, ydl_opts: dict) -> None:
        """Read cookie config and apply to yt-dlp opts. Prioritizes browser extraction over cookie string."""
        # Priority 1: cookiesfrombrowser (gets ALL cookies including httpOnly)
        browser = _cookie_manager.get_browser("bilibili")
        if browser:
            ydl_opts['cookiesfrombrowser'] = (browser,)
            logger.info(f"使用浏览器 Cookie: {browser}")
            return

        # Priority 2: cookie string from CookieConfigManager → Netscape file
        cookie_str = _cookie_manager.get("bilibili")
        if not cookie_str:
            logger.warning("B站 cookie 未配置，字幕获取可能失败")
            return

        tmp_path = Path(tempfile.gettempdir()) / "bilinote_bilibili_cookies.txt"
        tmp_path.write_text(
            self._cookie_string_to_netscape(cookie_str), encoding="utf-8"
        )
        ydl_opts['cookiefile'] = str(tmp_path)
        logger.info("已从 CookieConfigManager 加载 B站 cookie")
    ...
    def download(
        ...
        self._apply_bilibili_cookies(ydl_opts)
